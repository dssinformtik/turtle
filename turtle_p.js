...
var canx=getCanvasPos("c1").left, cany=getCanvasPos("c1").top, // canvas-Pos im Dokument (für Feinjustierungen)
	...
	groesse=104; // Voreinstellung, ändern mit sms.width=...+"px"

x+=tx; y+=ty;
if(y<0){ ty=-ty; y+=2*ty;} // alert(y+" , "+cany); 
if(y+groesse>Yw){ ty=-ty; y+=2*ty;} 
if(x<0){tx=-tx; x+=2*tx;} 
if(x+groesse>Xw){tx=-tx; x+=2*tx;} 
aufxy(x+groesse/2,y+groesse/2); kreis(5,360);

sms.left=canx+x+"px"; // Bildpos; Xw Canvas-Breite s.o.
sms.top=cany+y+"px"; 
setTimeout("meineFunktion()",20); // Delay; 0,9^LEV
